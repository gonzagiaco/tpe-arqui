...
public interface CarreraRepository extends RepoBase<Carrera, Long> {
    ...
    @Query("SELECT c.carrera, es.inscripcion, COUNT(DISTINCT es.estudiante.id) AS inscriptos, SUM(CASE WHEN es.graduado > 0 THEN 1 ELSE 0 END) AS egresados FROM Carrera c JOIN c.estudiantes es GROUP BY c.carrera, es.inscripcion ORDER BY c.carrera ASC, es.inscripcion ASC")
